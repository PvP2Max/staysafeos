...
export class ExportsController {
  ...
  private generateRidesCsv(rows: RideExportRow[]): string {
    ...
    const csvRows = rows.map((row) =>
      [
        ...
        this.escapeCsvField(row.requestTime),
        this.escapeCsvField(row.pickupTime),
  ...
  private async generateRidesExcel(rows: RideExportRow[]): Promise<Buffer> {
    ...
    worksheet.columns = [
      ...
      { header: "Request Time", key: "requestTime", width: 12 },
      { header: "Pickup Time", key: "pickupTime", width: 12 },
      { header: "Dropoff Time", key: "dropoffTime", width: 12 },
